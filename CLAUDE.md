...
plugins/plus/skills/<skill>/SKILL.md      工程、思考、工具、写作与 PUA Skills（19）
...
- 当前本地自有 Skill 是 `base/improve-codebase-architecture` 与 `plus/pua-loop`。
...
- `base` 固定 7 个 Skill、`plus` 固定 19 个；Skill 正文不得调用仓库不再发布的 Skill。
- `plus/agents` 仅对 Claude Code 生效；Pi 只加载 `plugins/*/skills`。
- `plus` 中的 PUA suite 只同步上游独立 Skills，不发布上游 commands、hooks、plugin-level Agents 或 scripts。
