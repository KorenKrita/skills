...
plugins/plus/skills/<skill>/SKILL.md      工程、思考、工具与写作增强（7）
plugins/plus/agents/*.md                  plus 的 Claude Code subagents
plugins/pua/skills/<skill>/SKILL.md       PUA 独立 Skills（12；无 commands/hooks/agents）
...
- 当前本地自有 Skill 是 `base/improve-codebase-architecture` 与 `pua/pua-loop`。
...
- `base` 固定 7 个 Skill、`plus` 固定 7 个、`pua` 固定 12 个；Skill 正文不得调用仓库不再发布的 Skill。
- `plus/agents` 仅对 Claude Code 生效；Pi 只加载 `plugins/*/skills`。
- `pua` plugin 只同步上游独立 Skills，不发布上游 commands、hooks、plugin-level Agents 或 scripts。
- 仓库只发布 `base`、`plus` 和 `pua` 三个 plugin。
