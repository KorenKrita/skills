...
- 是否要做已经定了，用户想要阶段、todo 或执行计划：用 description 明确匹配该任务的已安装执行规划 Skill。
- 想法已经成立，现在需要产品需求：用 description 明确匹配该任务的已安装产品规格 Skill。
- 用户想挑战范围或野心，不要 做/砍 的结论——打开格局、往大处想：用 `geju`。
- 问题纯粹是取名或改名：用 description 明确匹配该任务的已安装命名 Skill。
