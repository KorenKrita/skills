...
9. **为完整审计生成 HTML 交付物。** 路径：`/tmp/razor-<slug>/index.html`。在 Markdown 答案之后，写一份克制、可扫描的报告，包含结论、论据、落实前/落实后图、Razor Map、删减/合并清单、必须保留的复杂度、风险、护栏和下一步。如果只是很小的局部审计、HTML 比问题本身更重，可以跳过，但要说明为什么。生成了 HTML 时，回复里要给出它的绝对路径。
...
- 目标是需求文档或产品范围——用 description 明确匹配该任务的已安装产品规格 Skill。
- 目标是模块边界、依赖方向或抽象深度——用 description 明确匹配该任务的已安装架构 Skill。
- 问题是整个想法到底值不值得做——用 `idea`。
- 用户想把删减结果变成执行计划——用 description 明确匹配该任务的已安装执行规划 Skill。
...
仅在某个已安装 Skill 的 description 明确匹配下一项子任务时调用它；否则由当前工作流直接完成。任何未安装 Skill 都不能成为完成任务的前置条件。
