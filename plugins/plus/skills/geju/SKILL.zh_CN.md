...
- 问题是这件事到底值不值得做（做 / 砍 / 延后）——用 `idea`。geju 假定事情值得做，只质疑它的范围和高度。
- 用户想要实现质量审查——用 `clean-code-reviewer`。
- 用户想要 APoSD 级别的架构 critique（模块边界、抽象深度）——用 description 明确匹配该任务的已安装架构 Skill。
- 输出物是 PRD——用 description 明确匹配该任务的已安装产品规格 Skill。
