...
- The question is whether the idea is worth doing at all (do / kill / defer) — use `idea`. geju assumes the work is worth doing and questions only its scope and altitude.
- The user wants implementation-quality review — use `clean-code-reviewer`.
- The user wants APoSD-level architecture critique (module boundaries, abstraction depth) — use an installed architecture Skill whose description clearly matches the task.
- The output is a PRD — use an installed product-specification Skill whose description clearly matches the task.
