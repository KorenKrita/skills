...
- 真相来源是代码：用 `docs-vs-code` 检查文档是否符合实现、配置和 API 契约。本 skill 从不对照代码。
- 文档是 PRD，需要按产品规格打磨或修复（范围、验收、作为 spec 的结构）：用 description 明确匹配该任务的已安装产品规格 Skill。
- 文档只需要排版和呈现美化，而非一致性判断：用 `readme-beautifier`。
- 用户要的是按当前结论把文档重写出来，而不是一份问题报告：用 description 明确匹配该任务的已安装文档重写 Skill。本 skill 只诊断，不重写。
