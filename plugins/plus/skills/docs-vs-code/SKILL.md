...
description: |
  ...
  internal contradictions with no code comparison, use docs-vs-docs instead.
---
...
- The comparison is doc-vs-doc with no codebase as truth source (internal contradictions, stale sections, duplication) — use `docs-vs-docs`.
- You are auditing entity/data-model fields against a PRD (which fields exist, store-vs-compute, column-vs-config) — use an installed data-model auditing Skill whose description clearly matches the task.
- The document has drifted from the discussion conclusions and the user wants it rewritten, not a mismatch report — use an installed document-rewriting Skill whose description clearly matches the task.
