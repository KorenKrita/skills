...
- 对照对象是文档对文档、没有代码库作为真相来源（内部自相矛盾、过时段落、重复内容）——改用 `docs-vs-docs`。
- 审计实体/数据模型字段是否符合 PRD（字段是否该存在、该存还是该算、列还是配置）——改用 description 明确匹配该任务的已安装数据模型审计 Skill。
- 文档已经偏离讨论结论、用户要的是重写而不是一份不一致报告——改用 description 明确匹配该任务的已安装文档重写 Skill。
