...
4. ACT:  打开 PRIMARY MOD.md 并执行 ACTION REQUIRED
```
...
2. `skills/<PRIMARY>/MOD.md` — 进入目标模块并执行 ACTION REQUIRED
...
5. 分配角色（ops/role-map）；打开 PRIMARY MOD.md
...
- ❌ 不要沉默 — 遇到问题必须立即告知用户
- ❌ 不要自己编造工具版本号或功能描述
- ❌ 不要读完 RULES.md 后只回复"已理解，请告诉我具体任务" — 正确做法是运行 master-route.ps1 → 打开 PRIMARY MOD.md → 开始执行；仅在歧义时查 routing.md
...
4. 写入本包对应位置：
   ...
   - 特定工具用法 → 对应 skill 的 references/ 或 MOD.md
   - 踩坑经验 → field-journal/
...
新增后必须同步更新：routing-benchmark.json、routing.json、MASTER-ROUTING.md、skills/MOD.md；涉及工具时再更新 bootstrap-manifest.json、ToolDiscovery.ps1 和 refresh-tool-index.ps1。`routing.md` 仅作为歧义附录按需同步。
...
3. ACT: 打开 PRIMARY MOD.md；角色/时间线/证据链见 ops/*；身份见 ops/IDENTITY.md（不是 Z3r0 平台）
