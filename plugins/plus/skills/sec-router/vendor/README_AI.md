...
8. Open PRIMARY MOD.md → execute ACTION REQUIRED. Append timeline/workitems under the case dir.
...
└── skills\                       # Main skills directory
    ├── MOD.md                  # Main controller entry point
...
2. Go to `skills\MOD.md`
3. When handling a task, read files in this order:
   1. `MOD.md`
   2. `routing.md`
   3. The `MOD.md` in the corresponding subdirectory
...
- This package's `MOD.md`, `routing.md`, and `tool-index.md`
...
|---|---|---|
| Main controller entry | `MOD.md` | Read the global map first, then decide which sub-skill to enter |
...
- APK / Android → `apk-reverse\MOD.md`
- exe / dll / so / elf → `ida-reverse\MOD.md` or `radare2\MOD.md`
- Frontend signature / encrypted parameters → `js-reverse\MOD.md`
- HTTP capture / browser sampling / request replay → anything-analyzer, Reqable MCP + `js-reverse`
- Penetration testing / port scanning / vulnerability scanning → `pentest-tools\MOD.md`
- Firmware / IoT / router pentesting → `firmware-pentest\MOD.md`
- N-day / patch diff / CVE PoC writing → `patch-diff-exploit\MOD.md`
- Exploit writing / pwn / stack-heap-kernel exploitation → `pwn-chain\MOD.md`
- EDR / AV bypass / red-team delivery → `edr-bypass-re\MOD.md`
- Browser/desktop automation → `browser-automation\MOD.md`
- Symbol migration / cross-version comparison → `binary-diff\MOD.md`
- Diagrams / architecture diagrams / attack-path diagrams → `diagram-generator\MOD.md`
...
3. `MOD.md` — Main controller entry point
