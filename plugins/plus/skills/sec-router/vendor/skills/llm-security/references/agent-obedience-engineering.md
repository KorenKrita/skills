...
- 每个 MOD.md 的第一个 section 应该是"立即执行"而非"适用场景"
...
3. **NEXT**: 读取 `skills/MOD.md` → `skills/routing.md` → 确定进入哪个子 skill
...
- 禁止读完 RULES.md 后只回复"已理解，请告诉我具体任务"
  → 正确做法：执行全局注入 → 读 MOD.md → 读 routing.md → 确定入口
