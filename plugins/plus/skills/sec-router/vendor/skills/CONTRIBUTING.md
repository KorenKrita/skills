...
1. `MUST` 在 `MOD.md` 顶部加入 `ACTION REQUIRED` 区块，写清楚读完后立刻执行的 3-5 步。
2. `MUST` 在 `MOD.md` 末尾加入“任务完成自检”区块，未通过不得宣称完成。
...
每个新建 skill 的 `MOD.md` 中 `MUST` 包含语言行为契约，明确 AI 在内部推理和用户交互时的语言选择：
...
在 MOD.md 中把此模式放到真正有分岔的 decision boundary；不要机械地放到每个阶段末尾。
...
### 5.2 更新根 MOD.md / INDEX

打开 `skills/MOD.md` 模块表；跑 `extract-summaries.ps1` 重生 `INDEX.md`。
...
在 skill 的 `MOD.md` 中，必须包含一段"MCP 服务不可用时的手动配置指引"：
