...
$primary = 'reverse-engineering/MOD.md'
$primaryId = 'R0'
...
$timelineNext = if ($ready) {
    'open PRIMARY MOD.md and ACT within scope'
} else {
...
if ($ready) {
    Write-Host 'NEXT: open PRIMARY MOD.md and ACT within scope' -ForegroundColor Green
} else {
    Write-Host 'NEXT: fill scope.md auth + in_scope; then open PRIMARY MOD.md'
}
