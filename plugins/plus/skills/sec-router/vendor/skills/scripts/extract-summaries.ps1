...
# 生成 skills/INDEX.md：从各模块 MOD.md 的 frontmatter 提取 name+description，生成导航索引。
...
# 扫描版本库中的模块 MOD.md（排除本地 ignored/private trees，保证 clean clone 与开发机幂等）。
$skipDirs = @('ops', 'scripts', 'config', 'tests', 'field-journal', 'references')
$packageRoot = Split-Path -Parent $skillsRoot
$trackedSkillFiles = @()
$git = Get-Command git -ErrorAction SilentlyContinue
if ($git) {
    $trackedPaths = @(& $git.Source -C $packageRoot ls-files -- 'skills/**/MOD.md')
...
$candidateSkillFiles = if ($trackedSkillFiles.Count -gt 0) {
    ...
    @(Get-ChildItem -Path $skillsRoot -Recurse -Filter 'MOD.md')
}
$skillFiles = $candidateSkillFiles | Where-Object {
    $rel = $_.FullName.Substring($skillsRoot.Length + 1)
    $rel -ne 'MOD.md' -and -not ($skipDirs | Where-Object { $rel.StartsWith($_ + '\') -or $rel.StartsWith($_ + '/') })
...
[void]$sb.AppendLine('> 修改摘要请编辑对应模块 `MOD.md` 的 frontmatter `description`，然后重跑脚本。')
[void]$sb.AppendLine('')
