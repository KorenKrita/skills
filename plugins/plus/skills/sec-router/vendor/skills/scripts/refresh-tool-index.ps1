...
$scriptRefs = @{
    ...
    'rasm2' = @('radare2/MOD.md')
    'radiff2' = @('radare2/MOD.md')
    'rahash2' = @('radare2/MOD.md')
    'rax2' = @('radare2/MOD.md')
    'r2pm' = @('radare2/MOD.md')
    'r2xsql' = @('radare2/MOD.md')
    'r2xsql-full' = @('radare2/MOD.md')
    'r2mcp' = @('radare2/MOD.md')
    'radius2' = @('radare2/MOD.md')
    'python' = @('apk-reverse/scripts/frida-run.ps1', 'case-review/scripts/review_case.py')
    'pip' = @()
    'node' = @('js-reverse/MOD.md')
    'npx' = @('js-reverse/MOD.md')
    'jshookmcp' = @('js-reverse/MOD.md')
    'reqable-mcp' = @('pentest-tools/MOD.md')
    'xquik-mcp' = @('threat-intelligence/MOD.md')
    'jeb-pro' = @('apk-reverse/MOD.md')
    'seclists' = @('pentest-tools/MOD.md')
    'pentestswarm' = @('pentest-tools/MOD.md')
    'agent-browser' = @('browser-automation/MOD.md')
    'playwright' = @('browser-automation/MOD.md', 'browser-automation/scripts/setup.ps1')
    'analyzeHeadless' = @('reverse-engineering/MOD.md')
    'proxycat' = @('pentest-tools/MOD.md')
    'nmap' = @('pentest-tools/MOD.md')
    'binwalk' = @('firmware-pentest/MOD.md')
    'yara' = @('malware-analysis/MOD.md')
    'pwntools' = @('reverse-engineering/MOD.md', 'reverse-engineering/patterns-ctf*.md')
    'bkcrack' = @('reverse-engineering/crypto-decode-tools.md', '../CTF-Sandbox-Orchestrator/competition-zip-archive/MOD.md')
}
...
$reports = @(Get-ReverseToolReport | Where-Object {
    ...
    if ($keep -and -not [string]::IsNullOrWhiteSpace($skill)) {
        $skillMd = Join-Path $skillsRoot (($skill -replace '/', [IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar + 'MOD.md')
...
$markdownLines = @(
    ...
    '',
    "- 扫描时间: $generatedAt",
    '- 路由入口: `MOD.md` → `routing.md` → 对应子 skill',
...
$jsonPayload = [pscustomobject]@{
    ...
    routing_entry = @('MOD.md', 'routing.md')
    tools = $jsonRows
