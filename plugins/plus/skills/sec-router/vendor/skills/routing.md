...
6. After routing, read the target skill's MOD.md BEFORE taking action.
...
| WASM / Python bytecode / .NET / **DSL VM / 自定义虚拟机** | `reverse-engineering/dsl-vm-reverse/MOD.md` — IIFE + switch-case opcode JS VM | `reverse-engineering/languages.md` — real WASM binaries |
| Malware / virus sample | `malware-analysis/MOD.md` — six-stage + YARA/Sigma | `ida-reverse/` deep dive |
...
| **CTF competition (full stack)** | `../CTF-Sandbox-Orchestrator/ctf-sandbox-orchestrator/MOD.md` — master entry | Route to 40+ sub-skills by evidence |
| **CTF ZIP / PKZIP archive** | `../CTF-Sandbox-Orchestrator/competition-zip-archive/MOD.md` — legacy ZipCrypto + `bkcrack` known plaintext | Use before password brute force |
| Web runtime / API | `../CTF-Sandbox-Orchestrator/competition-web-runtime/MOD.md` | — |
| Cloud / Container / K8s | `cloud-k8s/` | CTF-only extra: sidecar orchestrator after PRIMARY `ctf-sandbox/` |
| Windows / AD / Identity | `../CTF-Sandbox-Orchestrator/competition-identity-windows/MOD.md` | — |
| Forensics / PCAP / Steganography | `../CTF-Sandbox-Orchestrator/competition-forensic-timeline/MOD.md` | — |
| Prompt injection / Agent | `../CTF-Sandbox-Orchestrator/competition-prompt-injection/MOD.md` | — |
| Mobile (Android/iOS) | `../CTF-Sandbox-Orchestrator/competition-android-hooking/MOD.md` | — |
| Firmware / Malware sample | `../CTF-Sandbox-Orchestrator/competition-firmware-layout/MOD.md` | — |

## By User Intent

| User Says | Route To |
|-----------|----------|
| "DSL VM / 自定义指令集 / 风控引擎逆向" | `reverse-engineering/dsl-vm-reverse/MOD.md` — IIFE + switch-case opcode |
| "fireye / fireyejs / getToken 逆向" | `reverse-engineering/dsl-vm-reverse/MOD.md` — runtime capture |
| "582KB JS 文件不是 WASM / 大 JS 文件逆向" | `reverse-engineering/dsl-vm-reverse/MOD.md` — classify DSL VM first |
| "decompile / IDA analyze" | `ida-reverse/MOD.md` — IDA MCP workflow |
| "recover source / disassemble" | `reverse-engineering/MOD.md` + `ida-reverse/` |
| "Frida hook / dynamic inject" | `reverse-engineering/tools-dynamic.md` — Frida section |
| "radare2 / r2 analyze" | `radare2/MOD.md` — CLI workflow |
| "find frontend signature / encrypted params" | `js-reverse/MOD.md` — Observe→Capture→Rebuild |
| "jshookmcp / JS hook / CDP debug" | `js-reverse/MOD.md` — same JS/Web chain |
| "Reqable / Reqable MCP / capture replay" | `pentest-tools/MOD.md` — authorized local capture and API workflow |
| "JEB / JEB Pro" | `apk-reverse/MOD.md` — licensed Android / ARM cross-check; verify local install first |
| "APK unpack / repack / modify smali" | `apk-reverse/MOD.md` — decode→rebuild-sign-install |
...
| "CTF challenge / competition reverse" | `ctf-sandbox/MOD.md` → sidecar orchestrator |
| "CTF ZIP / PKZIP / bkcrack / 压缩包明文攻击" | `../CTF-Sandbox-Orchestrator/competition-zip-archive/MOD.md` |
...
| "open webpage / browser automation / fill form" | `browser-automation/MOD.md` — Playwright |
| "crawl page / screenshot / auto login" | `browser-automation/MOD.md` |
| "desktop automation / Windows automation" | `browser-automation/MOD.md` — OpenReverse |
| "game reverse / anti-cheat / hack analysis" | `reverse-engineering/MOD.md` — game reverse (IL2CPP/Unity/Cheat Engine) |
| "Unity / IL2CPP / Mono" | `reverse-engineering/MOD.md` — Unity + `seed-014_unity-il2cpp-reverse.md` |
| "Cheat Engine / memory scan" | `reverse-engineering/MOD.md` — Cheat Engine memory analysis |
| "symbol migration / cross-version compare" | `binary-diff/MOD.md` — LLM batch migration |
| "missing PDB / old version symbols" | `binary-diff/MOD.md` — cross-version symbol migration |
| "bindiff / function offset migration" | `binary-diff/MOD.md` — binary diff |
| "port scan / Nmap" | `pentest-tools/MOD.md` — information gathering |
| "vulnerability scan / Nuclei" | `pentest-tools/MOD.md` — vulnerability detection |
| "SQL injection / SQLMap" | `pentest-tools/MOD.md` — web pentest |
| "directory brute force / FFUF / Gobuster" | `pentest-tools/MOD.md` — web pentest |
| "password cracking / Hashcat" | `pentest-tools/MOD.md` — password cracking |
| "penetration testing / active scan" | `pentest-tools/MOD.md` — pentest toolchain |
| "SRC hunting / Bug Bounty" | `pentest-tools/src-hunter/MOD.md` — 19 playbooks + H1 cases |
| "WAF bypass" | `pentest-tools/src-hunter/references/payloader/` — 263 bypass steps |
| "draw diagram / flowchart / architecture" | `diagram-generator/MOD.md` |
| "attack path diagram / sequence diagram" | `diagram-generator/MOD.md` — Mermaid/Graphviz/PlantUML |
| "malware / virus analysis / sample analysis" | `reverse-engineering/MOD.md` + YARA/sandbox |
...
| "cloud security / container escape / K8s" | `cloud-k8s/MOD.md` |
| "Prompt injection / AI security" | `llm-security/MOD.md` — OWASP LLM + ASI Top 10 |
| "internal network / lateral movement" | `pentest-tools/MOD.md` + `references/network-attack-defense.md` |
...
| "SSTI / template injection" | `pentest-tools/MOD.md` — SSTImap |
| "XSS scan / cross-site scripting" | `pentest-tools/MOD.md` — XSStrike |
| "WordPress pentest / WP enumeration" | `pentest-tools/MOD.md` — WPProbe |
| "C2 framework / adversary simulation" | `pentest-tools/MOD.md` — AdaptixC2 |
| "WiFi attack / wireless pentest" | `pentest-tools/MOD.md` — Fluxion + aircrack-ng |
| "NTLM relay / auth coercion" | `pentest-tools/MOD.md` — Coercer |
| "NetExec / CrackMapExec / nxc" | `pentest-tools/MOD.md` — network service enumeration |
| "AI auto pentest / MCP security" | `pentest-tools/MOD.md` — HexStrike AI / MetasploitMCP |
| "Swarm / swarm pentest / autonomous scan" | `pentest-tools/MOD.md` — Pentest Swarm AI |
| "red team / HW / attack exercise" | `attack-chain/MOD.md` — full attack chain orchestration |
| "initial breach / boundary breach" | `attack-chain/MOD.md` — boundary breach phase |
| "close-range pentest / BadUSB / WiFi phishing" | `attack-chain/MOD.md` — close-range section |
| "EDR bypass / evasion / AV bypass" | `edr-bypass-re/MOD.md` |
| "phishing / social engineering" | `email-security/MOD.md` |
| "supply chain attack" | `attack-chain/MOD.md` — supply chain section |
| "trace cleanup / anti-forensics" | `attack-chain/MOD.md` — cleanup section |
| "full pentest / end-to-end" | `attack-chain/MOD.md` — full chain planning |
| "from external to domain controller" | `attack-chain/MOD.md` — cross-phase path orchestration |
| "attack surface assessment / path planning" | `attack-chain/MOD.md` — path planning decision tree |
| "got shell, what next / post-exploitation" | `attack-chain/MOD.md` — plan from current foothold |
| "BurpSuite / Burp proxy / intercept" | `pentest-tools/MOD.md` + `references/burpsuite-mcp-guide.md` |
...
| "API security / GraphQL / JWT attack" | `api-security/MOD.md` — REST/GraphQL/JWT/OAuth |
| "supply chain security / SBOM / SCA" | `supply-chain-security/MOD.md` — Trivy/Syft/Gitleaks |
| "iOS reverse / IPA / Mach-O" | `mobile-reverse/MOD.md` — class-dump/Hopper/Frida iOS |
| "Objection / SSL Pinning bypass" | `mobile-reverse/MOD.md` — dynamic instrumentation |
| "YARA / malware detection rules" | `malware-analysis/MOD.md` — YARA/Sigma/IOC |
| "N-day / patch diff / CVE reproduction" | `patch-diff-exploit/MOD.md` |
...
| "pwn / stack overflow / ROP / ret2libc" | `pwn-chain/MOD.md` |
...
| "Hydra / online brute force" | `pentest-tools/MOD.md` — online password attack |
| "Metasploit / msfconsole / exploit" | `pentest-tools/MOD.md` — exploitation framework |
| "Wireshark / packet analysis / PCAP" | `digital-forensics/` or `protocol-reverse/` |
| "BurpSuite / web proxy / intercept" | `pentest-tools/MOD.md` — web proxy |
| "protocol reverse / Protobuf / gRPC / custom protocol" | `protocol-reverse/MOD.md` |
| "Ghidra / analyzeHeadless / no IDA" | `ghidra-reverse/MOD.md` |
| "Kubernetes / K8s / container escape / cloud IAM" | `cloud-k8s/MOD.md` |
| "Active Directory / Kerberoast / Certipy / BloodHound" | `windows-ad/MOD.md` |
| "forensics / Volatility / memory dump / IR timeline" | `digital-forensics/MOD.md` |
| "code audit / SAST / Semgrep / CodeQL / whitebox" | `code-audit/MOD.md` |
| "OSINT / threat intelligence / public X IOC enrichment" | `threat-intelligence/MOD.md` — public posts require independent corroboration |
| "threat hunting / blue team / detection engineering" | `threat-hunting/MOD.md` |
| "game reverse / IL2CPP / Unity / Unreal" | `reverse-engineering/MOD.md` + seed-014 |
| "Wi-Fi / aircrack / wireless pentest" | `wifi-wireless/MOD.md` |
| "browser extension / Chrome extension / crx" | `browser-extension-reverse/MOD.md` |
| "OT / ICS / SCADA / PLC / Modbus" | `ot-ics/MOD.md` |
| "macOS reverse / Mach-O / codesign" | `macos-reverse/MOD.md` |
| "thick client / desktop client / Electron security" | `thick-client/MOD.md` |
| "Go reverse / Rust reverse / GoReSym" | `go-rust-reverse/MOD.md` |
| "UART / JTAG / hardware debug pads" | `hardware-security/MOD.md` |
| "database security / Redis / Mongo / MSSQL hardening" | `database-security/MOD.md` |
| "phishing analysis / SPF DKIM DMARC / BEC" | `email-security/MOD.md` |
| "SAML / OIDC / SSO federation" | `identity-federation/MOD.md` |
| "SDR / HackRF / RF protocol research" | `radio-sdr/MOD.md` |
| "ProxyCat / proxy pool / IP rotation" | `pentest-tools/MOD.md` — proxy management |
...
|-----------|---------------------|----------|
| "unlock X / remove check / bypass detection" | Identify the check, explain control flow, propose local patch or input strategy | `reverse-engineering/MOD.md` → `ida-reverse/` or `radare2/` |
| "去除校验 / 解锁功能 / 绕过检测 / 去掉限制" | Same as above — locate check routine, document, propose patch | `apk-reverse/` (if APK) or `reverse-engineering/MOD.md` |
...
| "make it pass / 让我通过验证" | Recover validation logic, derive expected input or flag format | `reverse-engineering/MOD.md` |
...
DSL VM Reverse Path:
  reverse-engineering/dsl-vm-reverse/MOD.md → identify DSL VM (IIFE + single-letter vars + DG() switch-case)
  ↓ Extract opcode table & constant table
  reverse-engineering/dsl-vm-reverse/MOD.md Phase 2-4 → opcode classification, C[9] constant analysis
...
CTF Competition Path (via CTF-Sandbox-Orchestrator):
  ../CTF-Sandbox-Orchestrator/ctf-sandbox-orchestrator/MOD.md → build sandbox model
