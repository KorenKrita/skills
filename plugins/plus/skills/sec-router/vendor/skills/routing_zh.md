...
| 目标类型 | 推荐入口 | 备选方案 |
|---------|---------|---------|
| APK / Android 应用 | `mobile-reverse/MOD.md` — Frida/Objection/MobSF 全平台移动逆向 | `apk-reverse/` — 静态分析、jadx 反编译；可选已许可的 JEB Pro 交叉验证 |
| iOS / IPA 应用 | `mobile-reverse/MOD.md` — iOS 逆向 + Frida/Objection | `mobile-reverse/references/ios-reverse-guide.md` — iOS 专项 |
...
| 已有 case / 证据交接审查 | `case-review/MOD.md`: Evidence 图与 fixity 校验 | `docs-generator/`: 最终报告 |
...
| **CTF 竞赛全栈** | `../CTF-Sandbox-Orchestrator/ctf-sandbox-orchestrator/MOD.md` — 总控入口 | 按证据面路由到 40+ 子技能 |
| **CTF ZIP / PKZIP 压缩包题** | `../CTF-Sandbox-Orchestrator/competition-zip-archive/MOD.md` — legacy ZipCrypto + `bkcrack` 明文攻击 | 优先于密码暴力破解 |
| Web 运行时 / API | `../CTF-Sandbox-Orchestrator/competition-web-runtime/MOD.md` | — |
| 云 / 容器 / K8s | `../CTF-Sandbox-Orchestrator/competition-agent-cloud/MOD.md` | — |
| Windows / AD / 身份 | `../CTF-Sandbox-Orchestrator/competition-identity-windows/MOD.md` | — |
| 取证 / PCAP / 隐写 | `../CTF-Sandbox-Orchestrator/competition-forensic-timeline/MOD.md` | — |
| Prompt 注入 / Agent | `../CTF-Sandbox-Orchestrator/competition-prompt-injection/MOD.md` | — |
| 移动端 (Android/iOS) | `../CTF-Sandbox-Orchestrator/competition-android-hooking/MOD.md` | — |
| 固件 / 恶意样本 | `../CTF-Sandbox-Orchestrator/competition-firmware-layout/MOD.md` | — |
| **LLM 应用 / AI Agent** | `llm-security/MOD.md` — OWASP LLM + ASI Top 10 | `../CTF-Sandbox-Orchestrator/competition-prompt-injection/MOD.md` — CTF 场景 |
| **REST / GraphQL / WebSocket API** | `api-security/MOD.md` — 10 阶段方法论 | `pentest-tools/MOD.md` — 基础 Web 渗透 |
| **软件供应链 / SBOM / SCA** | `supply-chain-security/MOD.md` — 六层治理框架 | `pentest-tools/MOD.md` — 依赖扫描工具 |
| **恶意软件 / 病毒样本** | `malware-analysis/MOD.md` — 六阶段分析 + YARA/Sigma | `reverse-engineering/MOD.md` — 仅通用逆向 / `ida-reverse/` 深度分析 |
| **公开来源威胁情报 / OSINT** | `threat-intelligence/MOD.md` — IOC 补充与活动关联 | 公开 X/Twitter 帖子必须由独立来源核验 |

## 按用户意图

| 用户说 | 可以参考 |
|--------|---------|
| "反编译/IDA 看一下" | `ida-reverse/MOD.md` — IDA MCP 工作流 |
| "还原源码/还原为汇编/逆向还原" | `reverse-engineering/MOD.md` — 通用逆向 + `ida-reverse/` 或 capstone 静态反汇编 |
| "Frida hook 一下/动态注入" | `reverse-engineering/tools-dynamic.md` — Frida 章节 |
| "radare2 / r2 分析" | `radare2/MOD.md` — CLI 工作流 |
| "找前端签名/加密参数" | `js-reverse/MOD.md` — Observe→Capture→Rebuild |
| "jshookmcp / JS hook / CDP 调试" | `js-reverse/MOD.md` — 仍走同一条 JS/Web 逆向链路；调用前先确认该 MCP server 已下载、已注册到客户端、已启用 |
| "Reqable / Reqable MCP / 抓包重放" | `pentest-tools/MOD.md` — 授权范围内的本地抓包与 API 工作流 |
| "JEB / JEB Pro" | `apk-reverse/MOD.md` — 已许可的 Android / ARM 交叉验证；先确认本机安装 |
| "APK 解包/重打包/改 smali" | `apk-reverse/MOD.md` — decode→rebuild-sign-install |
...
| "审查 case / 证据链 / 可追溯性" | `case-review/MOD.md`: 只读 Evidence 图审查 |
| "写 writeup" | `docs-generator/` — CTF writeup 模板 |
| "打开网页/浏览器自动化/填表" | `browser-automation/MOD.md` — Playwright 浏览器操作 |
| "爬取页面/截图/自动化登录" | `browser-automation/MOD.md` — 浏览器自动化 |
| "Playwright / headless" | `browser-automation/MOD.md` — 浏览器自动化 |
| "操作桌面应用/Windows 自动化" | `browser-automation/MOD.md` — OpenReverse 桌面自动化 |
| "UIA/CUA/桌面 GUI 操作" | `browser-automation/MOD.md` — OpenReverse（UIA/CUA 模式） |
| "OpenReverse" | `browser-automation/MOD.md` — 桌面交互 + 网络观察 |
| "符号迁移/跨版本对比" | `binary-diff/MOD.md` — LLM 批量符号迁移 |
| "缺 PDB/旧版符号推导新版" | `binary-diff/MOD.md` — 跨版本符号迁移 |
| "bindiff/函数偏移迁移" | `binary-diff/MOD.md` — 二进制差分 |
| "N-day/补丁差分/CVE 还原/1day 武器化" | `patch-diff-exploit/MOD.md` — 补丁→PoC→打补丁前主机 |
...
| "pwn/栈溢出/ROP/ret2libc/写 exploit" | `pwn-chain/MOD.md` — RE→exploit 完整流水线 |
...
| "pwntools/GEF/pwndbg/one_gadget/libc-database" | `pwn-chain/MOD.md` |
| "固件渗透/路由器固件/IoT 漏洞利用" | `firmware-pentest/MOD.md` — 从提取打到实机 |
...
| "EDR 绕过/AV bypass/免杀/红队投递" | `edr-bypass-re/MOD.md` — 逆向防御方→针对性绕过 |
...
| "端口扫描/Nmap" | `pentest-tools/MOD.md` — 信息收集 |
| "漏洞扫描/Nuclei" | `pentest-tools/MOD.md` — 漏洞检测 |
| "SQL 注入/SQLMap" | `pentest-tools/MOD.md` — Web 渗透 |
| "目录爆破/FFUF/Gobuster" | `pentest-tools/MOD.md` — Web 渗透 |
| "密码破解/Hashcat" | `pentest-tools/MOD.md` — 密码破解 |
| "渗透测试/主动扫描" | `pentest-tools/MOD.md` — 渗透工具链 |
| "SRC 挖洞/Bug Bounty/众测" | `pentest-tools/src-hunter/MOD.md` — 19 类 playbook + H1 案例 |
| "WAF 绕过/bypass" | `pentest-tools/src-hunter/references/payloader/` — 263 绕过步骤 |
| "画图/流程图/架构图/攻击路径图" | `diagram-generator/MOD.md` — 图表生成 |
| "时序图/状态图/ER图/数据流图" | `diagram-generator/MOD.md` — Mermaid/Graphviz/PlantUML |
| "Mermaid/Graphviz/PlantUML" | `diagram-generator/MOD.md` — 图表生成 |
| "恶意软件/病毒分析/样本分析" | `malware-analysis/MOD.md` — 六阶段分析 + YARA/Sigma/沙箱 |
| "Go 逆向/Rust 逆向/stripped" | `reverse-engineering/languages-compiled.md` + `go-reverse.md` |
| "固件/IoT/binwalk/ARM" | `firmware-pentest/MOD.md` — 固件全链路分析（与按目标类型表一致） |
...
| "云安全/容器逃逸/K8s" | `../CTF-Sandbox-Orchestrator/competition-agent-cloud/MOD.md` |
| "Prompt 注入/AI 安全" | `llm-security/MOD.md` — OWASP LLM + ASI Top 10 完整框架 |
...
| "内网渗透/横向移动" | `pentest-tools/MOD.md` + `references/network-attack-defense.md` |
...
| "SSTI/模板注入" | `pentest-tools/MOD.md` — SSTImap 自动检测 |
| "XSS 扫描/跨站脚本" | `pentest-tools/MOD.md` — XSStrike 高级扫描 |
| "WordPress 渗透/WP 枚举" | `pentest-tools/MOD.md` — WPProbe 插件枚举 |
| "C2 框架/对抗模拟/AdaptixC2" | `pentest-tools/MOD.md` — AdaptixC2 后渗透与对抗模拟框架 |
| "Atomic Red Team/检测测试" | `pentest-tools/MOD.md` — Atomic-Operator |
| "WiFi 攻击/无线渗透" | `pentest-tools/MOD.md` — Fluxion + aircrack-ng |
| "NTLM relay/认证强制" | `pentest-tools/MOD.md` — Coercer |
| "WinRM/Windows 远程" | `pentest-tools/MOD.md` — evil-winrm-py |
| "NetExec/CrackMapExec/nxc" | `pentest-tools/MOD.md` — 网络服务枚举 |
| "AI 自动渗透/MCP 安全" | `pentest-tools/MOD.md` — HexStrike AI / MetasploitMCP / mcp-kali-server |
| "Swarm/群体渗透/自主扫描" | `pentest-tools/MOD.md` — Pentest Swarm AI（pentestswarm scan --swarm） |
| "Bug Bounty 自动化/持续监控" | `pentest-tools/MOD.md` — Pentest Swarm AI playbook: bug-bounty |
| "攻击面管理/ASM" | `pentest-tools/MOD.md` — Pentest Swarm AI playbook: external-asm |
| "红队/攻防演练/HW" | `attack-chain/MOD.md` — 完整攻击链编排（信息收集→突破→提权→横向→维持） |
| "打点/初始突破/边界突破" | `attack-chain/MOD.md` — 边界突破阶段 |
| "近源渗透/BadUSB/WiFi钓鱼" | `attack-chain/MOD.md` — 近源渗透章节 |
| "投递免杀/实战绕过EDR/Shellcode加载器" | `attack-chain/MOD.md` — 攻击链中的 EDR/AV 绕过（实战投递阶段） |
| "钓鱼/社工/邮件钓鱼" | `attack-chain/MOD.md` — 钓鱼攻击章节 |
| "供应链攻击" | `attack-chain/MOD.md` — 供应链攻击章节 |
| "痕迹清理/反取证" | `attack-chain/MOD.md` — 痕迹清理章节 |
| "完整渗透测试/全流程" | `attack-chain/MOD.md` — 全链路规划 |
| "从外网打到域控/内网" | `attack-chain/MOD.md` — 跨阶段路径编排 |
| "攻击面评估/攻击路径规划" | `attack-chain/MOD.md` — 路径规划决策树 |
| "拿到 shell 下一步/后渗透" | `attack-chain/MOD.md` — 从当前据点规划后续 |
| "内网渗透全流程" | `attack-chain/MOD.md` — 横向移动 + 提权 + 域攻击 |
...
| "Hydra/在线爆破/SSH 爆破" | `pentest-tools/MOD.md` — 在线密码爆破 |
| "Nikto/Web 服务器扫描" | `pentest-tools/MOD.md` — Web 漏洞扫描 |
| "Metasploit/msfconsole/exploit" | `pentest-tools/MOD.md` — 利用框架 |
| "Wireshark/抓包分析/PCAP" | `digital-forensics/` 或 `protocol-reverse/` |
| "协议逆向/Protobuf/自定义协议" | `protocol-reverse/MOD.md` |
| "Ghidra/无 IDA" | `ghidra-reverse/MOD.md` |
| "K8s/容器逃逸/云安全" | `cloud-k8s/MOD.md` |
| "域渗透/BloodHound/Certipy/Kerberoast" | `windows-ad/MOD.md` |
| "取证/Volatility/内存转储" | `digital-forensics/MOD.md` |
| "代码审计/SAST/Semgrep" | `code-audit/MOD.md` |
| "开源情报/威胁情报/公开 X IOC 补充" | `threat-intelligence/MOD.md` — 公开帖子仅作为待核验线索 |
| "威胁狩猎/蓝队/检测工程" | `threat-hunting/MOD.md` |
| "游戏逆向/IL2CPP/Unity" | `reverse-engineering/MOD.md` + seed-014 |
| "WiFi/无线渗透/aircrack" | `wifi-wireless/MOD.md` |
| "浏览器扩展/Chrome 扩展/crx" | `browser-extension-reverse/MOD.md` |
| "工控/OT/ICS/SCADA/PLC" | `ot-ics/MOD.md` |
| "macOS逆向/Mach-O" | `macos-reverse/MOD.md` |
| "厚客户端/桌面客户端" | `thick-client/MOD.md` |
| "Go逆向/Rust逆向" | `go-rust-reverse/MOD.md` |
| "UART/JTAG/硬件调试" | `hardware-security/MOD.md` |
| "数据库安全/Redis/Mongo" | `database-security/MOD.md` |
| "钓鱼邮件/SPF/DKIM/DMARC" | `email-security/MOD.md` |
| "SAML/OIDC/SSO联邦" | `identity-federation/MOD.md` |
| "SDR/射频/HackRF" | `radio-sdr/MOD.md` |
| "BurpSuite/Web 代理/拦截" | `pentest-tools/MOD.md` — Web 代理 |
| "Responder/LLMNR 投毒/NBT-NS" | `pentest-tools/MOD.md` — 内网投毒 |
| "BloodHound/AD 路径/攻击图" | `pentest-tools/MOD.md` — AD 攻击路径可视化 |
| "Certipy/AD CS/证书攻击" | `pentest-tools/MOD.md` — AD 证书服务攻击 |
| "wfuzz/参数模糊/Web Fuzz" | `pentest-tools/MOD.md` — Web 模糊测试 |
| "GDB/GEF/调试/断点" | `reverse-engineering/tools.md` — 动态调试 |
| "objdump/反汇编/ELF 分析" | `reverse-engineering/MOD.md` — 静态分析 |
| "strings/字符串提取" | `reverse-engineering/MOD.md` — 快速侦察 |
| "ProxyCat/代理池/IP 轮换" | `pentest-tools/MOD.md` — 代理管理 |
| "LLM 安全/AI 安全测试/Prompt 注入测试" | `llm-security/MOD.md` — OWASP LLM + ASI Top 10 完整框架 |
...
| "garak/PyRIT/AI 红队" | `llm-security/MOD.md` — LLM 安全工具链 |
| "API 安全测试/接口渗透" | `api-security/MOD.md` — 10 阶段 API 测试方法论 |
...
| "BOLA/IDOR/BFLA/对象级授权绕过" | `api-security/MOD.md` — Phase 3 授权测试 |
| "供应链安全/SBOM/SCA/依赖扫描" | `supply-chain-security/MOD.md` — 六层供应链治理 |
| "CI/CD 安全/管道审计/构建完整性" | `supply-chain-security/references/cicd-pipeline-security.md` — 管道安全 |
| "容器安全/镜像扫描/Trivy/Cosign" | `supply-chain-security/MOD.md` — 容器安全章节 |
| "gitleaks/密钥扫描/凭证泄漏" | `supply-chain-security/MOD.md` — CI/CD 管道安全 |
| "iOS 逆向/IPA/Objective-C/Swift/Mach-O" | `mobile-reverse/MOD.md` — iOS 逆向 + Frida/Objection |
...
| "移动安全测试/MSTG/OWASP Mobile" | `mobile-reverse/MOD.md` — OWASP MASTG 方法论 |
...
| "IOC 提取/威胁情报/恶意软件分析" | `malware-analysis/MOD.md` — 六阶段分析流程 |
...
CTF 竞赛路径（通过 CTF-Sandbox-Orchestrator）：
  ctf-sandbox-orchestrator/MOD.md → 建立沙盒模型
  ↓ 按主导证据面路由
...
N-day 武器化路径：
  ...
  ↓ 武器化
  pwn-chain/MOD.md（构造稳定 exploit）+ pentest-tools/references/msf-protocol.md（Metasploit 模块化）

红队投递路径：
  attack-chain/MOD.md → 选阶段
  ↓ 需要绕 EDR
