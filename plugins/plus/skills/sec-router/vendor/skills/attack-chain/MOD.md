...
| 需求 | 路由到 |
|------|--------|
| Web 漏洞深度利用 | `pentest-tools/MOD.md` |
| 内网 AD 攻击详细步骤 | `windows-ad/MOD.md` |
| 逆向分析恶意样本 | `reverse-engineering/MOD.md` |
| APK 逆向（移动端渗透） | `apk-reverse/MOD.md` |
| JS 前端签名绕过 | `js-reverse/MOD.md` |
| 自动化群体渗透 | Pentest Swarm AI（`pentestswarm scan --swarm`） |
| AI 辅助渗透 | `mcp-kali-server` / `metasploitmcp` / `hexstrike-ai` |
| 报告生成 | `docs-generator/MOD.md` |
| 攻击路径图 | `diagram-generator/MOD.md` |
