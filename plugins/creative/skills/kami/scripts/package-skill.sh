...
awk '
  ...
  /^scripts\/(build|check-update|checks|content|ensure-fonts|highlight|lint|mermaid_normalize|optional_deps|render|shared|site_facts|tokens|verify|visual)\.(py|sh)$/ { print; next }
