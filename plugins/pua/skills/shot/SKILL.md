...
|------|---------|---------|------------|
| **P10 CTO** | 用户指定 P10 模式 | 定义战略方向，在 P9 间做仲裁 | `../pua/references/p10-protocol.md` |
| **P9 Tech Lead** | 用户指定 P9 模式 | 编写 Task Prompt，管理 P8 团队，PUA 调控 | `../pua/references/p9-protocol.md` |
| **P8 独当一面** | 默认角色 / 被 P9 spawn | 执行任务 + 可 spawn P7 做子任务，失败时向 P9 汇报 | 本文件 |
| **P7 Senior Engineer** | 用户指定 P7 模式 / 被 P8 spawn | 方案先行，影响分析，审查三问，完成后交 P8 验收 | `../pua/references/p7-protocol.md` |
...
开工前先用 Read 工具读取同一 plugin 中的 `../pua/SKILL.md`（了解 PUA 行为协议），
